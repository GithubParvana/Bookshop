...
@app.route('/')
def home_page():
    return render_template('index.html', category=category, best=bestsellers, recommend=recommended_items)



@app.route('/products')
def products_():
    return render_template('product.html', pro=book_list, category=category)
...
def books(id):
    print(id)
    books = book_list[id]
    return render_template('book.html', book=books, recommend=recommended_items, lists=book_list, rew=reviews)
